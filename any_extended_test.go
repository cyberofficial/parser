...
// Test for performance with larger datasets
func Test_ANY_Performance(t *testing.T) {
	// Create a larger dataset
	var largeItems []struct {
		ID    int
		Tags  []string
		Score int
	}

	// Generate 100 items with varying tags
	tags := []string{"tag1", "tag2", "tag3", "tag4", "tag5", "tag6", "tag7", "tag8", "tag9", "tag10"}

	for i := 0; i < 100; i++ {
		// Each item gets between 1-5 random tags
		itemTags := []string{}
		numTags := (i % 5) + 1
		for j := 0; j < numTags; j++ {
			tagIndex := (i + j) % len(tags)
			itemTags = append(itemTags, tags[tagIndex])
		}

		largeItems = append(largeItems, struct {
			ID    int
			Tags  []string
			Score int
		}{
	...
	// Perform some queries that would exercise the ANY operator
	queries := []struct {
		name  string
		query string
	}{
		...
		{
			name:  "ANY with multiple values",
			query: "ANY(Tags) = ANY('tag1', 'tag2', 'tag3')",
		},
		{
			name:  "Complex ANY with AND",
			query: "ANY(Tags) = ANY('tag1', 'tag2') AND Score > '500'",
		},
		{
			name:  "Complex ANY with OR",
			query: "(ANY(Tags) = 'tag1' OR ANY(Tags) = 'tag10') AND Score < '300'",
		},
	}

	for _, q := range queries {
		t.Run(q.name, func(t *testing.T) {
			_, err := Parse(q.query, largeItems)
			if err != nil {
				t.Fatalf("Error parsing performance query %q: %v", q.query, err)
			}
			// We don't assert specific result counts for performance tests
		})
	}
}
